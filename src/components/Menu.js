    ...
    return (
        <>
            <div className="hidden md:p-5 md:flex flex-col justify-between w-1/6 h-screen">
                ...
                <button className="hidden md:flex justify-between items-center w-full bg-slate-100 hover:bg-slate-200 focus:outline-none font-medium rounded-full px-5 py-2.5 text-sm text-center text-black">
                    <div className="flex flex-col items-start">
                        <p className="font-bold">{userInfo.username}</p>
                        <p className="text-gray-400">@{userInfo.tagname}</p>
                    </div>
                    <PropertiesBtn />
                </button>
            </div>
            <div id="mask_div" className={`absolute z-10 bg-black w-screen h-screen top-0 left-0 opacity-80 ${showPostForm ? 'md:block' : 'hidden'} `}></div>
